import java.util.*;
//compile javac PHPArray.java -Xlint
//get a ton of warnings...but no errors!
//javac Assig2.java
//then run Java Assig2
...
public class PHPArray<V> implements Iterable<V> {
   ...
   //  globals.........
    ...
    // hashTable= array structure for hashed Pair Objects
   //   Nodes= root, tail : for linked list implementation
   //   Nodes=   next, prev : for rehashing Node from mid  list
    ...
    // Instantiate new PHPArray and capacity set
    ...
    public Iterator<V> iterator() {
        ...
        Iterator<V> newIter = new Iterator<V>() {
    ...
    public Iterator<Pair<V>> pairIterator() {
        ...
        Iterator<Pair<V>> newIter = new Iterator<Pair<V>>() {
     ...
     * @return null if it is at end of table or table = empty
     */
    public Pair<V> each() {
        if (iter == null) {
            reset();
    ...
    // Holds Key Value pairs for each node. Public static inner class allows it to be nested within PHPArray. it can still be 
    // accessed by Pair<V> which has two public instance variables:   key of type String and value of type V
    ...
    /**
     * Node contains next & prev node references for double L-list 
    ...
    /**
     * Accepts String parameter and generates unique hash value bitwise anded and % by table size. 
     * @param key
     * @return a unique hash value from table
     */
    ...
    /**
     * creates new Node<V> object containing Pair<V> which contains the passed Key Value pair--- hashes object within the hashTable and adds to 
     * internal linked list. If values are for reHash, utilizes global next and prev references to replace into same spot in list. 
     * @param key
     * @param val  */
    ...
    /**
     * overloaded put() method accepts integer key and converts it to a string
    ...
    /**
     * returns value of specified key by grabbing hash value and comparing to  corresponding table index
     * @param key
    ...
    public V get(String key) {
     ...
     /**
     * returns key of specified value by grabbing hash value and comparing to corresponding table index if the key exists
     * @param key
     * @return V value of specified Key, null if not found*/
    ...
    /**
     * removes key value pair and node from table and ArrayList if the  Node containing the Key exists. If gap created in cluster, rehashes
    ...
    public void unset(String key) {
        ...
        // delete key and the associated value it has
        if (root == tail) {
            root = null;
        ...
        while (hashTable[i] != null) {
            ...
            hashTable[i] = null;
            N--;
            reHash = true;  //set glob reHash boolean to rehash link
    ...
    /**
     * doubles table size by rehashing all Nodes within temp PHPArray object and copying to current PHPArray object
    ...
    /**
     * Overloaded unset() method accepts int key and converts to Integer object which is converted to a string and passed to unset(String Key)
     * @param key */
    ...
    /**
     * @param key searching..
     * @return true if key contained within table, false if not in table*/
    ...
    // key and value of all Pair<V> objects of all the Nodes
    ...
    /**
     * Sorts insertion order linked nodes from smallest to largest, utilizes merg_sort() which is worst case O(NlgN). reHashes in insertion
    ...
    /**
     * Merge Sort is good sort that sorts table thru Linked nodes in O(NlgN) runtime
     * @param n
     * @return */
    ...
    /**
     * merges on the way up from bottom of recursion tree
    ...
    /**
     * finds middle node of linked list for splitting purposes
